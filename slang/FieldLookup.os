...
public object FieldLookup {
    ...
    public Vector<FieldEntry> getParameters( int databaseHandle, string entityName ) const throws {
        var query = "SELECT *
                       FROM information_schema.parameters
                      WHERE SPECIFIC_SCHEMA = '" + Config.Database + "'
                        AND SPECIFIC_NAME = '" + entityName + "'";
        ...
        var result = mysql_store_result( databaseHandle );
        while ( mysql_fetch_row( result ) ) {
            var column = mysql_get_field_value( result, "PARAMETER_NAME" );
            var type   = mysql_get_field_value( result, "DATA_TYPE" );
